$(function () {
	...
	$(window).scroll(() => {
		if ($(window).scrollTop() > 0) {
			...
			$('.menu .ham6').css('margin', '0px');

			if ($('.menu .ham6').hasClass('active')) {
				$('.menu .ham6 .line').css('stroke', '#000');
			} else {
				$('.menu .ham6 .line').css('stroke', '#fff');
			}
		} else {
	...
	$('.ham6').click(function () {
		...
		if (checked) {
			...
			$('.menu-button svg').css('margin-right', `${scrollWidth}px`);
			$('.background').css('padding-right', `${scrollWidth}px`);

			TweenLite.to('.background', 2, { ease: Power3.easeOut, autoAlpha: 1 });
			TweenLite.to('.background .links', 1.2, { ease: Power3.easeOut, y: 0, delay: 0.5 });
			tl.staggerTo('.background .links li', 0.4, { autoAlpha: 1, delay: 0.5 }, 0.1);

			if ($(window).scrollTop() > 0) $('.ham6 .line').css('stroke', '#000');
		} else {
			$('body').css('overflow-y', 'scroll');
			$('body').css('padding-right', '0px');
			$('.menu-button svg').css('margin-right', '0px');
			$('.background').css('padding-right', '0px');

			TweenLite.to('.background', 2, { ease: Power3.easeOut, autoAlpha: 0, delay: 0.15 });
			TweenLite.to('.background .links', 1, { y: 20 });
			tl.staggerTo('.background .links li', 0.4, { autoAlpha: 0 }, 0.1);

			if ($(window).scrollTop() > 0) $('.ham6 .line').css('stroke', '#fff');
		}
	});
